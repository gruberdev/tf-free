...
<Tabs items={['Free Resource list', 'Limits and Usage Policies', 'Requirements', 'More Information', 'Cleaning Up']}>
  <Tab>
  ...
  <Tab>
    ...
    - S3 Bucket size limits for the free tier are not enforced. This means manual tracking is required to avoid paying fees.
  </Tab>
  <Tab>
  ...
  <Tab>
    <Callout type="error" emoji="️🚫">
      **CAUTION**: The steps below will delete every resource, machine and configuration in your AWS account. 
    ...
    You will need to install a tool called [awsweeper][awsweeper-uri]. To install the latest version, go to their [releases page][awsweeper-releases] and download the binary, or run this command:
    ```bash filename="Any compatible POSIX terminal with curl installed"
    curl -sSfL https://raw.githubusercontent.com/jckuester/awsweeper/master/install.sh | sh -s
    ```
    You will also need `task` to installed, run the following command while inside the project's root directory:
    
    ```bash
    task clean:aws
    
    # Or if you rather run the command manually
...
[awsweeper-uri]: https://github.com/jckuester/awsweeper
[awsweeper-releases]: https://github.com/jckuester/awsweeper/releases