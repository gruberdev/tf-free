...
<Tabs items={['Free Resource list', 'Limits and Usage Policies', 'Required APIs', 'More Information', 'Cleaning Up']}>
  <Tab>
  ...
  <Tab>
   - Requires a valid credit card associated with your account, even if you don't pay for using any of the resources available.
   ...
   - Storage size limits are not enforced on Storage Buckets, you will have to manually keep a track of how much you have used in order to avoid paying fees.
  ...
  <Tab>
    <Callout type="error" emoji="️🚫">
      **CAUTION**: The steps below will delete every resource, machine and configuration in your GCP project. 

      Only follow up through these instructions if you are aware of what this might cause and it is your intent to destroy all resources, effectively cleaning up your account.
      
      This should only be done when if you have already run `terraform destroy` successfully and it wasn't able to delete everything. At some extreme cases it might be an alternative to reset corrupt states of your Terraform configuration, however this is not recommended.
    </Callout>
    You will need a tool called [Cloudblaster][gcp-clean-uri], a local environment properly initialized with your Google account using [gcloud's SDK cli tool][gcloud-sdk-cli]
    and a working container runtime (such as Rancher Desktop, nerdctl or Docker).

    After initializing your user on `gcloud` with:
    ```bash
    gcloud auth login
    ``` 
    Run `Cloudblaster`, run the following command replacing `Docker` with your tool of choice equivalent:
...
[gcp-clean-uri]: https://github.com/doitintl/CloudBlaster
